...
d0 <- merge(NEI[, c("SCC", "Emissions", "year")], SCC[, c("SCC", 
  "EI.Sector")], by = "SCC")
d0 <- subset(d0, grepl(".*Coal", EI.Sector))[, list(total = sum(Emissions)), 
  by = year]

## Plot the data, facet by Data.Category, and set labels.
print(qplot(year, total, data = d0, geom = "line", main = "Total PM2.5 emissions Country-wide\nproduced by coal combustion-related sources", 
