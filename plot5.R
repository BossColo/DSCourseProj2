...
BaltCity <- subset(NEI, fips == "24510", c('SCC', 'Emissions', 'year'))
...
d0 <- subset(d0, grepl(".*Vehicle.*", SCC.Level.Two))[, list(total=sum(Emissions)), by=list(year, SCC.Level.Two)]

## Plot the data grouped only by year, and set labels.
print(qplot(year, total, data=d0[, list(total=sum(total)), by=year], geom='line',
            main='Total PM2.5 emissions in Baltimore City, Maryland produced by motor vehicle sources', ylab='Total emissions (tons)', xlab='Year'))
